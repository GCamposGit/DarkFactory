...
- Anthropic / Claude Code: localiza o binário/CLI `claude` (no PATH, em `$APPDATA\npm\claude.cmd`, em `$LOCALAPPDATA\Programs\Claude\claude.exe`, ou em `~/.local/bin/claude`) e executa `claude auth status --json` para consultar a sessão autenticada da CLI local, extraindo email e plano configurados. Caso o binário não esteja disponível ou a execução falhe, cai para leitura da credencial em `~/.claude/.credentials.json`, detectando a presença de sessão autenticada do Claude Code local e reportando a conta como conectada.
