...
def test_all_script_tags_share_cache_version_20260923c():
    """Verify that every script tag in index.html shares the expected ?v=20260923c cache version."""
    html = INDEX_HTML.read_text(encoding="utf-8")
...
def test_stylesheet_link_uses_cache_version_20260923c():
    """Verify that the stylesheet link in index.html uses ?v=20260923c."""
    html = INDEX_HTML.read_text(encoding="utf-8")
    assert '<link rel="stylesheet" href="/static/styles.css?v=20260923c">' in html, (
        "index.html must link styles.css with ?v=20260923c"
    )
