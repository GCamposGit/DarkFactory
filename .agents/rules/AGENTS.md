...
- **Autonomia Total de Git em Projetos Internos (Zero Toque Humano Pós-Grill, USR-57)**: É terminantemente proibido orientar o usuário a executar commits, merges ou rotinas manuais de sincronização Git no terminal. Em projetos internos (`project: darkfac`), o agente e a linha de produção realizam o ciclo completo de forma 100% autônoma (commit atômico, merge e sincronização com `origin/main` via `core.git.autonomy`), atualizando a demanda para `completed`. Aprovações humanas pré-merge só são exigidas em projetos comerciais com a flag `requires_commercial_acceptance: true` onde há clientes externos em produção real.
